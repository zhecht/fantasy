...
def rankings_route():
	...
	for position in ["qb", "rb", "te", "wr"]:
		...
		for site in ["yahoo", "espn", "fantasypros"]:
			graph = {"title": "Wk{} {} % Err Vs. Projected Rank [{}]".format(curr_week, site, position), "position": position, "site": site, "actual": [], "projected": [], "err": [], "perc_err": [], "name": []}
			...
			for player in arr:
				try:
					if player["position"] == position:
						...
						graph["perc_err"].append(str(perc_err))
						total_err += err
